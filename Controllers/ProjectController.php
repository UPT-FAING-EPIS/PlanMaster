...
class ProjectController {
    ...
    public function __construct() {
        ...
        $this->porterAnalysis = new PorterAnalysis();
    }
    ...
    // ============= MÉTODOS PARA MATRIZ DE PORTER =============

    // Guardar análisis de Matriz de Porter
    public function savePorterAnalysis() {
        AuthController::requireLogin();
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            try {
                // Inicializar análisis Porter si no existe
                $this->porterAnalysis->initializeForProject($project_id);
                
                // Procesar datos de factores si se enviaron
                if (isset($_POST['factors_data'])) {
                    $factorsData = json_decode($_POST['factors_data'], true);
                    if ($factorsData) {
                        $this->porterAnalysis->saveAnalysis($project_id, $factorsData);
                    }
                }
                
                // Procesar oportunidades y amenazas si se enviaron
                if (isset($_POST['oportunidades']) && isset($_POST['amenazas'])) {
                    $oportunidades = json_decode($_POST['oportunidades'], true) ?: [];
                    $amenazas = json_decode($_POST['amenazas'], true) ?: [];
                    
                    $this->porterAnalysis->saveFodaItems($project_id, $oportunidades, $amenazas);
                }
                
                echo json_encode(['success' => true, 'message' => 'Análisis Porter guardado exitosamente']);
    ...
    // Obtener análisis de Porter de un proyecto
    public function getPorterAnalysis($project_id) {
        // Inicializar si no existe
        $this->porterAnalysis->initializeForProject($project_id);
        return $this->porterAnalysis->getByProject($project_id);
    }
    
    // Obtener puntuación y recomendaciones Porter
    public function getPorterScore($project_id) {
        return $this->porterAnalysis->calculateScore($project_id);
    }
    
    // Obtener oportunidades y amenazas Porter
    public function getPorterFoda($project_id) {
        return $this->porterAnalysis->getFodaItems($project_id);
    }
    
    // Verificar si el análisis Porter está completo
    public function isPorterComplete($project_id) {
